...
const arrayWordReducer = (state = defaulArrayWord, action) => {
    if(action.type === 'MEMORISED_CHANGE') return state.map(e=>{
        if(e.id !== action.id) return e;
        return {...e, memorized: !e.memorized}
    });
    if(action.type === 'MEANING_CHANGE') return state.map(e =>{
        if(e.id !== action.id) return e;
...
export default arrayWordReducer;